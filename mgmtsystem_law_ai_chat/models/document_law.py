...
class DocumentLaw(models.Model):
    ...
    ai_quest_id = fields.Many2one(comodel_name='ai.quest',string="",help="", copy=False)

    @api.onchange("stage")
    def _onchange_stage_id(self):
        if self.stage not in ['draft', 'cancel', 'done']:
            if self.ai_quest_id:
    ...
    def write(self, vals):
        ...
        if self.stage not in ['draft', 'cancel', 'done']:
            if not self.ai_quest_id:
                self.ai_quest_id = self.env['ai.quest'].create({
                ...
                self.ai_quest_id.channel_id = self.env['discuss.channel'].create({
                    'name': f"{self.rss_titel[:20]}",
                    'ai_quest_id': self.ai_quest_id.id,
